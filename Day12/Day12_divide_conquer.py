"""
Day 12 — 分治思想：二分查找 + 快速排序 + 归并排序 + LeetCode 704/912
======================================================================
每个函数里留了 TODO 让你实现，跑通后去掉 TODO 注释。

参考答案在 Day12_divide_conquer_答案.py，做完再对照。
"""
...
def partition(arr: list, low: int, high: int) -> int:
    """分区函数：选 arr[high] 为 pivot，把小的放左边、大的放右边。
    返回 pivot 最终位置的索引。

    示例：
    >>> arr = [5, 3, 8, 1, 9, 2]
    >>> idx = partition(arr, 0, 5)
    >>> arr
    [1, 2, 8, 5, 9, 3]   # 2 归位，左边都<2，右边都>2（不保证有序）
    """
    # TODO: 实现 partition
    pivot = arr[high]
    i = low - 1
...
def quick_sort(arr: list, low: int, high: int) -> None:
    ...
    [1, 2, 3, 5, 8, 9]
    """
    # TODO: 实现 quick_sort，调用 partition
    if low<high:
...
def search_704(nums: list, target: int) -> int:
    ...
    给定 n 个元素有序的升序数组 nums 和目标值 target，
    返回 target 的索引，不存在则返回 -1。

    >>> search_704([-1, 0, 3, 5, 9, 12], 9)
    4
    >>> search_704([-1, 0, 3, 5, 9, 12], 2)
    -1
    """
    # TODO: 实现（可直接调 binary_search）
...
def sort_array_912(nums: list) -> list:
    ...
    给你一个整数数组 nums，升序排列后返回。

    >>> sort_array_912([5, 2, 3, 1])
    [1, 2, 3, 5]
    >>> sort_array_912([5, 1, 1, 2, 0, 0])
    [0, 0, 1, 1, 2, 5]
    """
    # TODO: 用快速排序或归并排序实现
    if len(nums)<=1:
...
if __name__ == "__main__":
    ...
    ms_cases = qs_cases  # 同样的测试用例
    total += len(ms_cases)
    for arr, expected in ms_cases:
